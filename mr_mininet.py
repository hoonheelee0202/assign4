...
def parseCmdLineArgs ():
    ...
    parser.add_argument ("-W", "--worker", type=int, default=1, help="Number of Workers, default 2")
...
def runMapReduceWordCount (hosts, args):
    try:
        ...
        print "Running cmd: python ./mr_wordcount.py -m ", str (args.map), " -r ", str (args.reduce), " ", args.datafile, " on host: ", hosts[0].IP()
        ...
        popens [ hosts[0] ] = hosts[0].popen ('python', './mr_wordcount.py', '-m', str(args.map), '-r', str(args.reduce), args.datafile, stdout=subprocess.PIPE )
...
def genCommandsFile (hosts, args):
    try:
        ...
        #cmds.write (cmd_str)
				...
				worker_port_start = 8081
        #  next create the command for the map workers
        for i in range (args.worker):
        		cmd_str = hosts[i+1].name + " /usr/lib/jvm/java-8-openjdk-amd64/bin/java -cp /opt/spark/conf/:/opt/spark/jars/* -Xmx1g org.apache.spark.deploy.worker.Worker --webui-port "+ (worker_port_start+i) + " spark:" + hosts[0].IP () + ":7077 &"
            cmds.write (cmd_str)
...
def main ():
    ...
    topo = MR_Topo (Racks=parsed_args.racks, W = parsed_args.worker)
